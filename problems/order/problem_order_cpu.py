...
def linear_solver(pi, D, alpha=1):
    ...
    A = torch.zeros(D.size(0), D.size(0))
    w = torch.pow(D, -alpha)
    ...
    row1 = torch.zeros(D.size(0)*D.size(0))
    row2 = torch.zeros(D.size(0) * D.size(0))
    col1 = torch.zeros(D.size(0)*D.size(0))
    col2 = torch.zeros(D.size(0) * D.size(0))
    values1 = torch.zeros(D.size(0)*D.size(0))
    values2 = torch.zeros(D.size(0) * D.size(0))
...
class Order(object):
    ...
    def get_costs(dataset, pi, metric='tsp'):
        ...
        if metric == 'stress':
            ...
            image_data = image_data[:pi.size(0)].reshape(pi.size(0), n1, pa[1]*pa[2]*pa[3])
            d_image = image_data.gather(1, pi.unsqueeze(-1).expand_as(image_data)).reshape(pi.size(0), pa[0], pa[1], pa[2], pa[3])
